<template>
    <div class="paddingTop search_page">
        ...
        <form class="search_form">
            <input type="search" name="search" placeholder="请收入商家或者美食名称" class="search_input" v-model="searchValue" @input="checkInput">
                ...
                <router-link :to="{path:'/shop',query:{id:item.id}}" tag="li" v-for="item in restaurantList" :key="item.id" class="list_li">
                    ...
                    <section class="item_right">
                        <div class="item_right_text">
                            ...
                            <p>月首 {{item.month_sales || item.recent_order_num}} 单</p>
                            <p>{{item.delivery_fee || item.float_minimum_order_amount}} 元起送 / 距离{{item.distance}}</p>
                        </div>

                        <ul class="item_right_detail">
                            <li v-for="activities in item.restaurant_activity" :key="activities.id">
                                <span :style="{backgroudColor:'#'+activities.icon_color}" class="activities_icon">{{activities.icon_name}}</span>
...
export default {
  ...
  methods: {
    async searchTarget(historyValue) {
      ...
      console.log('historyValue',this.searchValue);
    ...
    clearAllHistory(){
        this.searchHistory = [];
        setStore('searchHitory',this.searchHistory);
    }
